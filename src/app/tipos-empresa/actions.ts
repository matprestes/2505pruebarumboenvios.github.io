...
export async function getTiposEmpresaAction({
  ...
  if (error) {
    console.error("Error fetching tipos_empresa:", error);
    return { data: [], count: 0 };
  }
...
export async function getTipoEmpresaByIdAction(id: string): Promise<TipoEmpresa | null> {
  ...
  if (error) {
    console.error("Error fetching tipo_empresa by ID:", error);
    return null;
  }
...
export async function addTipoEmpresaAction(
  ...
  const supabase = createClient();
  const validatedFields = tipoEmpresaSchema.safeParse(values);
  ...
  if (error) {
    console.error("Error adding tipo_empresa:", error);
...
export async function updateTipoEmpresaAction(
  ...
  if (error) {
    console.error("Error updating tipo_empresa:", error);
    return { success: false, message: `Error al actualizar tipo de empresa: ${error.message}` };
  }
  revalidatePath("/tipos-empresa");
  revalidatePath(`/tipos-empresa/${id}`); // If a detail page exists
...
export async function deleteTipoEmpresaAction(id: string): Promise<{ success: boolean; message: string }> {
  ...
  if (error) {
    console.error("Error deleting tipo_empresa:", error);
    // Consider checking for FK constraints error (e.g., if tipos_empresa are in use)
