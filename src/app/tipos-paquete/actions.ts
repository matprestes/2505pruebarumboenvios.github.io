...
export async function addTipoPaqueteAction(
  ...
  const supabase = createClient();
  // id_tipo_paquete and created_at are auto-generated by DB
  // activo has a default in DB, so it's optional from form if not explicitly set
  const { id_tipo_paquete, created_at, ...insertData } = values;
  const validatedFields = tipoPaqueteSchema
    .omit({ id_tipo_paquete: true, created_at: true })
    .safeParse(insertData);
...
export async function updateTipoPaqueteAction(
  id_tipo_paquete_param: string,
  values: z.infer<typeof tipoPaqueteSchema>
): Promise<{ success: boolean; message: string; tipoPaquete?: TipoPaquete }> {
  const supabase = createClient();
  // Exclude created_at and id_tipo_paquete from update payload
  const { created_at, id_tipo_paquete, ...updateData } = values;
  const validatedFields = tipoPaqueteSchema
    .omit({id_tipo_paquete: true, created_at: true })
  ...
  const { error, data } = await supabase
    ...
    .eq("id_tipo_paquete", id_tipo_paquete_param)
    .select()
    .single();
